...
Status: `MAP / P1–P4 CLOSED / H5 DIAGNOSTIC`
...
| cycle ≠ deadlock | residual-feasible cycle | C1 / CE-C1; H2-v2 `residual_cycle` | scientifically closed |
...
| P2c `IMS-SIP^1` siphon dual | unit-capacity one-hold/one-request bijection | C0; CE-SIP1; H2-v2 4 agrees + 4 refusals | closed inside `IMS-SIP^1` only |
| P3d/P3e exact thresholds | preregistered grids | BIX1-SAT; BIX2 32-row grid | closed for those families; PO-T4-6e still open |
| A2b local noncompletion | request-closed local kernel, outside job still moves | article A2b; H4-v3 | constructively closed |
...
| exact/DES same-target | 18 predeclared cells | article-core v1; H4-v3 6/6 | closed as within-case cross-check |
...
| CRP local bridge | four independent Prop 6.4 fields | G5 falsified; H5 scores fields, field 4 refused | **not a positive journal result** |
| unselected closed class in island CTMC | classified `R_*` or certified `S_T` | G5 grid/medium inconclusive; H4-v3 certified | **closed for the v3 island only** |
...
| same-semantics baselines | cycle / closed-core / siphon-or-refuse / LTS truth | H2-v2 4 agrees + 4 refusals, FP=0 FN=0 | **closed in h2_v2** |
| scale / diversity | parameterized enumerable or typed refusal | H3-v2 16/19; 12×≥1e3; 4×≥1e4 | **closed in h3_v2** |
| realistic manufacturing KPI | documented island + intervention + predeclared KPI | H4-v3 base \(\theta_b=1\), drain \(\theta_b=0\) | **closed in v3** |
...
T-ASE hardening witnesses H1–H4, H2-v2, H3-v2, and the H5 four-field
diagnostic now exist on this worktree. The remaining manuscript gap is
IEEE body packaging, not another science family. G5 FAIL and G6-B
`OPEN_PENDING` stay retained.
